(function(root){
  ...
  function calculate(raw,smooth=false){
    const loans=normalize(raw),N=Math.max(...loans.map(l=>l.n));
    ...
    for(let m=1;m<=N;m++){
      ...
      for(let i=0;i<loans.length;i++){
        ...
        if(smooth&&i===anchor&&payments[i]<intr-1e-7)throw Error('infeasible');
      ...
      rows.push({month:m,payments,payment:payments.reduce((a,b)=>a+b,0),interest,principal,balance:balances.reduce((a,b)=>a+b,0)});
    }
    ...
    return {rows,periods,total,totalPrincipal,interest:total-totalPrincipal,months:N,anchor};
  }
  function solve(raw,target){
    ...
    for(let m=1;m<=N;m++){
      ...
      rows.push({month:m,payments,payment:payments.reduce((s,p)=>s+p,0),interest,principal,balance});
    }
  ...
  root.LoanMath={factor,monthlyRate,normalize,calculate,solve};
