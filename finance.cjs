...
const context={Intl};vm.createContext(context);vm.runInContext(fs.readFileSync(require('node:path').join(__dirname,'../dist/i18n.js'),'utf8')+';globalThis.data={LANGUAGES,CURRENCIES,KEYS,SOLVER_KEYS,GLOBAL_KEYS,I18N,NOTES}',context);const d=context.data;assert.equal(d.LANGUAGES.length,11);assert.equal(d.CURRENCIES.length,30);for(const [language]of d.LANGUAGES){assert.equal(Object.keys(d.I18N[language]).length,d.KEYS.length+d.SOLVER_KEYS.length+d.GLOBAL_KEYS.length+4);assert(d.NOTES[language].length===4);for(const key of [...d.KEYS,...d.SOLVER_KEYS,...d.GLOBAL_KEYS,'loan','periodInterest','negativeAmortization','flexibleAmounts'])assert(d.I18N[language][key],`${language}.${key}`);}
