...
r=check([loan(1000,24),loan(100000,12)],true);assert(r.adaptedTerms);close(r.rows[0].payment,101000/24);
r=check([loan(260000,12,12,'years'),loan(180000,24,8,'years'),loan(120000,36,4,'years')],true);
assert(r.adaptedTerms);assert.deepEqual(r.effectiveTerms,[432,432,432]);assert.equal(r.periods.length,1);
...
const context={Intl};vm.createContext(context);vm.runInContext(fs.readFileSync(require('node:path').join(__dirname,'../dist/i18n.js'),'utf8')+';globalThis.data={LANGUAGES,CURRENCIES,KEYS,SOLVER_KEYS,GLOBAL_KEYS,I18N,NOTES}',context);const d=context.data;assert.equal(d.LANGUAGES.length,11);assert.equal(d.CURRENCIES.length,30);for(const [language]of d.LANGUAGES){assert.equal(Object.keys(d.I18N[language]).length,d.KEYS.length+d.SOLVER_KEYS.length+d.GLOBAL_KEYS.length+5);assert(d.NOTES[language].length===4);for(const key of [...d.KEYS,...d.SOLVER_KEYS,...d.GLOBAL_KEYS,'loan','periodInterest','negativeAmortization','flexibleAmounts','adaptedTerms'])assert(d.I18N[language][key],`${language}.${key}`);}
