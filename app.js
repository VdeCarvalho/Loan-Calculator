...
document.addEventListener('click',async e=>{
  const button=e.target.closest('#download-excel');if(!button||!state.result||button.disabled)return;
  const words={solveFor:t('solveFor'),target:targetLabel(state.target),mode:t('mode'),modeName:t(state.mode),currency:t('currency'),monthlyPayment:t('monthlyPayment'),loan:t('loan'),amount:t('amount'),duration:t('duration'),durationUnit:t('years')+'/'+t('months'),rate:t('rate'),rateUnit:t('annual')+'/'+t('monthly'),borrowed:t('borrowed'),interest:t('interest'),total:t('total')};
  button.disabled=true;button.textContent=t('generating');$('#excel-error').textContent='';
  try{await LoanExcel.download(state,words,LoanMath);}catch(error){console.error(error);if($('#excel-error'))$('#excel-error').textContent=t('excelError');}
  finally{button.disabled=false;button.textContent=t('downloadExcel');}
});
