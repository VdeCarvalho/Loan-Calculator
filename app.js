...
const globalRule=()=>state.target==='duration'?'commonDuration':state.target==='rate'?'commonRate':'equalAmounts';
...
function results(){
  ...
  return `<section class="result" aria-labelledby="result-heading"><div class="result-top"><div><div class="eyebrow" id="result-heading">${targetLabel(state.target)}</div><div class="payment">${primaryResult(r)}</div>${state.target==='payment'?`<div class="hint">${periodName(r.periods[0])}</div>`:''}</div><span class="badge">${t(state.mode==='smooth'?'smooth':'standard')}</span></div>${r.solved?`${r.solved.length>1?`<div class="solved-list">${r.solved.map((l,i)=>`<div><span>${t('loan')} ${number(i+1)}</span><strong>${solvedValue(l,i)}</strong></div>`).join('')}</div>`:''}<p class="hint">${t(state.target==='duration'?'durationNote':state.target==='rate'?'rateNote':globalRule())}</p>`:''}${r.hasNegativeAmortization?`<p class="amortization-notice" role="note">${t('negativeAmortization')}</p>`:''}<dl class="stats"><div><dt>${t('borrowed')}</dt><dd>${money(r.totalPrincipal)}</dd></div><div><dt>${t('interest')}</dt><dd>${money(r.interest)}</dd></div><div><dt>${t('total')}</dt><dd>${money(r.total)}</dd></div></dl></section><section class="card"><h2>${t('phases')}</h2>${r.periods.map(p=>`<div class="period"><div class="period-label"><span>${periodName(p)}</span><div class="period-bar"><i style="width:${Math.max(2,p.payment/Math.max(...r.periods.map(x=>x.payment))*100)}%"></i></div></div><strong>${money(p.payment)} <small>${t('perMonth')}</small></strong></div>`).join('')}</section><section class="card table-card" aria-labelledby="schedule-title"><div class="table-header"><h2 id="schedule-title">${t('schedule')}</h2><span class="hint">${number(r.months)} ${t('months').toLowerCase()}</span></div><div id="table-area">${table()}</div></section><button type="button" class="calculate download" id="download-pdf">${t('downloadPdf')}</button><p class="error" id="pdf-error" role="alert"></p>`;
}
function table(){
...
function render(){
  ...
  $('#app').innerHTML=`<header><h1 class="brand"><img src="favicon.svg" alt="">Loan Calculator</h1></header><div class="layout">${ad('left')}<main><form id="calculator" class="card" novalidate><div class="preferences"><div><label for="language">${t('language')}</label><select id="language">${LANGUAGES.map(([code,name])=>`<option value="${code}" ${code===state.lang?'selected':''}>${name}</option>`).join('')}</select></div><div><label for="currency">${t('currency')}</label><select id="currency">${CURRENCIES.map(code=>`<option value="${code}" ${code===state.currency?'selected':''}>${code} — ${escapeHTML(names.of(code))}</option>`).join('')}</select></div></div><div class="target-control"><label for="solve-for">${t('solveFor')}</label><select id="solve-for">${targets.map(target=>`<option value="${target}" ${state.target===target?'selected':''}>${targetLabel(target)}</option>`).join('')}</select></div>${inverse()?`<div class="global-payment"><label for="global-payment">${t('globalPayment')}</label><div class="amount-wrap"><input id="global-payment" type="text" inputmode="decimal" value="${escapeHTML(state.payment)}" required><span class="currency-tag">${state.currency}</span></div><p class="hint">${t(globalRule())}</p>${state.target!=='duration'?`<p class="hint">${t('globalNormal')}</p>`:''}</div>`:''}<div class="section-title"><h2>${t('loans')}</h2></div><div id="loans">${state.loans.map(loanForm).join('')}</div><button class="add" id="add-loan" type="button">＋ ${t('add')}</button>${state.target==='duration'?'':`<div class="mode-row"><span class="label" id="mode-label">${t('mode')}</span><div class="segments" role="radiogroup" aria-labelledby="mode-label">${['standard','smooth'].map(mode=>`<label><input type="radio" name="mode" value="${mode}" ${state.mode===mode?'checked':''}>${t(mode)}</label>`).join('')}</div>${state.mode==='smooth'?`<p class="hint">${NOTES[state.lang][1]}</p>`:''}</div>`}<div class="error" role="alert" id="error">${state.error?t(state.error):''}</div><button class="calculate" type="submit">${t('compute')} <span aria-hidden="true">↗</span></button></form><div id="results" aria-live="polite">${results()}</div><details class="card"><summary>${t('assumptions')}</summary>${[...NOTES[state.lang].slice(0,3),t('durationNote'),t('rateNote')].map(p=>`<p>${p}</p>`).join('')}<a href="https://en.wikipedia.org/wiki/List_of_languages_by_total_number_of_speakers" target="_blank" rel="noopener noreferrer">${t('source')} ↗</a></details><footer>${t('footer')}</footer></main>${ad('right')}</div>`;
}
...
function pdfReport(){
  const r=state.result,inputs=state.loans.map((l,i)=>{
    ...
    return [t('loan')+' '+number(i+1),t('amount')+': '+money(solved?solved.p:Number(numeric(l.amount))),t('duration')+': '+durationText(solved?solved.n:Number(numeric(l.duration))*(l.durationUnit==='years'?12:1),l.durationUnit),t('rate')+': '+rateText(solved?solved.r:LoanMath.monthlyRate(Number(numeric(l.rate)),l.rateUnit),l.rateUnit)];
  });
  const summary=[[t('solveFor'),targetLabel(state.target)],[t('currency'),state.currency],[t('mode'),t(state.mode==='smooth'?'smooth':'standard')],[t('borrowed'),money(r.totalPrincipal)],[t('interest'),money(r.interest)],[t('total'),money(r.total)]];
  if(inverse())summary.push([t('globalPayment'),money(Number(numeric(state.payment)))]);
  ...
  if(state.mode==='smooth')notes.push(NOTES[state.lang][1]);
  if(r.hasNegativeAmortization)notes.push(t('negativeAmortization'));
  ...
  if(state.loans.length>1)state.loans.forEach((l,i)=>tables.push({title:t('loan')+' '+number(i+1)+' · '+state.currency,headers:[t('month'),t('payment'),t('balance')],widths:[.14,.43,.43],rows:r.rows.map(row=>[number(row.month),money(row.payments[i]),money(row.loanBalances[i])])}));
  return {lang:state.lang,rtl:['ar','ur'].includes(state.lang),downloadLabel:t('downloadPdf'),title:t('resultLabel'),date:new Intl.DateTimeFormat(state.lang,{dateStyle:'long'}).format(new Date()),summary,inputs,notes,tables};
}
