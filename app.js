...
function pdfReport(){
  ...
  const summary=[[t('currency'),state.currency],[t('mode'),t(state.mode==='smooth'?'smooth':'standard')],[t('borrowed'),money(r.totalPrincipal)],[t('interest'),money(r.interest)],[t('total'),money(r.total)]];
  ...
  return {lang:state.lang,rtl:['ar','ur'].includes(state.lang),downloadLabel:t('downloadPdf'),title:t('resultLabel'),primary:{label:targetLabel(state.target),value:primaryResult(r).replace(/<[^>]+>/g,'')},date:new Intl.DateTimeFormat(state.lang,{dateStyle:'long'}).format(new Date()),summary,inputs,notes,tables};
}
